...
void tft(void *pvParameters)
{
	...
	spi_master_init(&dev_tft, CONFIG_MOSI_GPIO, CONFIG_SCLK_GPIO, CONFIG_CS_GPIO, CONFIG_DC_GPIO, CONFIG_RESET_GPIO, CONFIG_BL_GPIO);
	lcdInit(&dev_tft, CONFIG_WIDTH, CONFIG_HEIGHT, CONFIG_OFFSETX, CONFIG_OFFSETY);
...
TickType_t display_string_tc(char text[]) {
    ...
    int centerY = (height / 2) - ((strlen(text) * fontWidth) / 2) ;
